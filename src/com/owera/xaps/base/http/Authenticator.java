...
public class Authenticator {
	...
	private static String computeBlockedClientKey(HttpServletRequest req) {
		...
		String username = authorization; // use raw header if basic auth, since the header is always the same
		int startPos = authorization.indexOf("username=") + 9;

		if (startPos > 8) { // digest auth, must extract username, since header varies in every request
	...
	public static boolean authenticate(HTTPReqResData reqRes) throws TR069AuthenticationException {
		...
		// Code for early return of a non-authorized device with no logging or fuss - minimize impact of 
		// devices which constantly tries to log on to Fusion (without success).
		// If a device performs 5 non-authorized login attempts with less than 5 minutes
		// between each attempt, the device will be blocked by this code. The only way for the
		// device to be allowed into verification is to be silent for more than 5 minutes. 
