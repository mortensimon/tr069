...
public class GPVDecision {
	...
	private static void oppositePriorityProvisioning(HTTPReqResData reqRes) {
		 ...
		 * true when no parameter will be transmitted to the CPE. However, a password
		 * can complicate this, if a device do not support ParameterKey feature and/or
		 * the passwords have not set the C-flag on the unittype parameter. This
		 * introduces some level of dependency both on the device-implementation and
		 * on the setup of the unittype.
		 * 
		 ...
		 * "waiting" the next periodic inform interval will be calculated as usual. In
		 * other words, the service-window which those other methods might need
		 ...
		 * window). The question is if this in itself is so troubling that this whole
		 * idea must be abandoned. Perhaps the most reasonable solution is to
		 ...
		 * easy-provisioning. Service Windows never been used AFAIK - so this may
		 * be a fair trade. To accomplish this change, we need to change the job table -
		 * adding service-window there - which we cannot do in 2013R1. Thus 
		 * we should wait with this until 2014R1.
		 */
	...
	private static void processPeriodic(HTTPReqResData reqRes) throws TR069Exception {
		...
		if (!sessionData.isJobUnderExecution()) {
			// update unit-parameters with data from CPE, to get correct group-matching in job-search
			// will not affect the comparison in populateToCollections()
			updateUnitParameters(sessionData);
			try {
				uj = JobLogic.checkNewJob(Module.TR069, sessionData); // may find a new job
			} catch (SQLException sqle) {
				throw new TR069DatabaseException(sqle);
			}
		}
		Job job = sessionData.getJob();
		if (job == null) { // No job is present - process according to profile/unit-parameters
			//			String priority = sessionData.getOweraParameters().getValue(SystemParameters.PROVISIONING_PRIORITY);
			//			if (priority != null && priority.equals("Config-Download-Reboot-Reset")) {
			//				oppositePriorityProvisioning(reqRes); // Not implemented - does normal priority-prov
			//			} else {
			normalPriorityProvisioning(reqRes);
			//			}
		} else {
	...
	private static void jobProvisioning(HTTPReqResData reqRes, Job job, UnitJob uj) throws TR069Exception {
		...
		if (type == JobType.RESET) {
			...
			ShellJobLogic.execute(reqRes, sessionData, job, uj);
			reqRes.getResponse().setMethod(TR069Method.SET_PARAMETER_VALUES);
			return;
			...
			prepareSPVForConfigJob(sessionData, job);
			reqRes.getResponse().setMethod(TR069Method.SET_PARAMETER_VALUES);
			return;
		}
	}
	...
	private static void prepareSPVForConfigJob(SessionData sessionData, Job job) throws TR069Exception {
	...
	//		try {
	//			InetAddress address = InetAddress.getByName(Properties.getSyslogServer(Module.TR069));
	//			DatagramSocket socket = new DatagramSocket();
	//			StringBuffer sb = new StringBuffer();
	//			sb.append("<133>Jan  1 00:00:00 cpe [" + sessionData.getOweraParameters().getValue(SystemParameters.MAC) + "]:TR-metrics:");
	//			for (ParameterValueStruct pvs : sessionData.getToSyslog()) {
	//				UnittypeParameters unittypeParameters = sessionData.getUnittype().getUnittypeParameters();
	...
	//			socket.close();
	//		} catch (Throwable t) {
	//			Log.warn(GPVDecision.class, "Transmission of metrics to syslog server failed:", t);
	//		}
	//
	//	}

	/**
	 * Loop through all parameters defined in the database, and see which ones
	 * are missing in the CPE. We only do this if the GPV has been run twice,
	 * clearly indicating that the first GPV resulted in FA, something that CAN
	 * happen because we ask for a parameter from the CPE which is not there.
	 * Usually this will happen because the firmware is no longer in sync with
	 * the unittype definition.
	 */
	...
	private static void populateToCollections(SessionData sessionData) {
		...
		//		List<ParameterValueStruct> toSyslog = new ArrayList<ParameterValueStruct>();
		...
		for (int i = 0; i < sessionData.getFromCPE().size(); i++) {
			...
			//				Log.warn(GPVDecision.class, msg(utp, cpeV, acsV, "Ignored", "Inspection-flag & mode is PERIODIC"));
			//			else 
			if (acsV != null && !acsV.equals("ExtraCPEParam") && !(acsV.equals(cpeV))) {
				if (utp.getFlag().isReadWrite()) {
					...
					//						if (sessionData.getJobParams() != null && sessionData.getJobParams().get(utp.getName()) != null) {
					//							Log.debug(GPVDecision.class, msg(utp, cpeV, acsV, "CPE->SYS", "Param has ReadOnly- and Monitored-flag and is requested in a (monitor-)job"));
					//							toSyslog.add(pvsCPE);
					//						}
					//						if (utp.getFlag().isAlwaysRead()) {
					//							Log.debug(GPVDecision.class, msg(utp, cpeV, acsV, "CPE->ACS", "Param has ReadOnly-, Monitored- and AlwaysRead-flag"));
					//							toDB.add(pvsCPE);
					//						}
					//					} else {
